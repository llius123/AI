---
name: figma-to-plan
description: "Generates a development plan from structured Figma data, mapping components to Prisma Design System. Use when you have analyzed Figma JSON and need an implementation plan."
---
...
Generates a development plan in markdown from structured Figma data, mapping components to Prisma Design System.

## When to Use This Skill

Use this skill when:
- You have structured Figma JSON (generated by figma-data-analyzer)
- You need an implementation plan with Prisma Design System components
- You want to see the visual mapping from Figma → your component library
- You need example JSX code and folder structure

## Capabilities

- Analyze structured Figma JSON
- Map Figma components to Prisma Design System by semantic type
- Generate visual layout in ASCII
- Create example JSX code with correct imports
...
Accept:
- Path to JSON file from figma-data-analyzer OR raw JSON string
...
- Hierarchy/structure information (from `uiStructure.frames`, `uiStructure.textElements`)
- Button labels and groups (from `uiStructure.textElements` and `uiStructure.frames`)
- Layout patterns (from `uiStructure.frames[].layoutMode` - HORIZONTAL/VERTICAL)
...
Before proposing any component, prop, or value, read the actual library source:
...
3. If a component or prop **cannot be confirmed** → mark it with `⚠️ UNVERIFIED` in the mapping table and in the JSX as a comment
...
### 3. Figma → Prisma Design System Mapping
Table with columns:
- Figma Element (name from Figma)
- Prisma Component (mapped component)
- Inferred Props (inferred props — only verified ones)
- Notes (special considerations, ⚠️ UNVERIFIED if applicable)
...
- [ ] Implement base component with suggested JSX
- [ ] Define props interface (TypeScript)
- [ ] Map design tokens (colors, typography)
- [ ] Implement unit tests
- [ ] Create Storybook story
- [ ] Review accessibility (ARIA labels, etc.)
...
- Suggest running figma-data-analyzer again
...
- Can use @implementing-code to generate the actual component code
